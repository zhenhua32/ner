    ...
    with torch.no_grad():
        for batch, (input_ids, attention_mask, token_type_ids, inputs_seq_len, output_seq) in enumerate(dataloader):
            ...
            if use_crf:
                ...
                preds_seq_batch = model.crf.decode(pred, mask=(input_ids != 0).byte())
            else:
                pred, _ = model(input_ids, attention_mask, token_type_ids, output_seq)
            ...
            for pred_seq, gold_seq, input_seq, l in zip(
                ...
                # 从数字索引转换成标签
                # 从 1 开始，因为 0 是 [CLS] 的位置
                pred_seq = [i2w_bio[i] for i in pred_seq[1 : 1 + l]]
                gold_seq = [i2w_bio[i] for i in gold_seq[1 : 1 + l]]
                # char_seq = [tokenizer.convert_ids_to_tokens(i) for i in input_seq[1 : 1 + l]]
                char_seq = tokenizer.convert_ids_to_tokens(input_seq[1 : 1 + l])
                # TODO: 这里需要验证下
